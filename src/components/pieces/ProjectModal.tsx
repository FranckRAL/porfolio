...
import { ProjectFromDB, ProjectView } from "@/types";
...
const ProjectModal = ({
  ...
  const challenge = project.challenges
  ...
  return (
    <div className="fixed inset-0 z-100 flex items-center justify-center p-4 md:p-10">
      ...
      <div
        className="absolute inset-0 bg-abyss-950/90 backdrop-blur-xl animate-in fade-in duration-300"
        onClick={onClose}
      />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 p-6 md:p-12">
          ...
          <div className="space-y-6">
            ...
            <div className="p-6 bg-primary/5 rounded-3xl border border-primary/10">
              <h4 className="text-sm font-mono text-primary uppercase tracking-widest mb-4">
                Tech Stack
              </h4>
              <div className="flex flex-wrap gap-4">
                {project.stack.map((tech) => (
                 ...
                 transition-all duration-300 disabled:opacity-70 text-text-main text-xs"
                  >
                    {tech}
                  </p>
                ))}
          ...
          <div className="space-y-8 py-2">
            ...
            <div className="space-y-4">
              ...
              <ul className="grid grid-cols-1 gap-3">
                {features.map((f, i) => (
                  <li
                    key={i}
                    className="flex items-start gap-3 text-text-muted bg-primary/5 p-3 rounded-xl border border-primary/5"
                  >
            ...
            {challenge && (
              <div className="p-6 bg-orange-500/5 border border-orange-500/20 rounded-3xl">
                <h4 className="text-sm font-bold text-orange-400 uppercase mb-2">
                  Technical Challenge
                </h4>
            ...
            <div className="flex flex-wrap gap-4 pt-6">
              ...
              {!project.liveUrl && !project.githubUrl && (
                <Link
                  href="#contact"
                  onClick={onClose}
                  className="bg-primary text-abyss-950 px-8 py-4 rounded-2xl font-bold hover:opacity-90 transition-opacity"
                >
                  {t("cta_contact")}
                </Link>
              )}
